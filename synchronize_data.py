...
file_c=open(calib_file, 'w',encoding='UTF8')
writer_file=csv.writer(file, delimiter=',')
writer_calib=csv.writer(file_c, delimiter=',')
        ...
        if timestamp==timestamps[0]:
          if row[0]!='time':
            if float(row[0])<timestamp:
              calib_data=[row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],row[15],row[16],row[17],row[18],row[19],row[20],row[21],row[22],row[23],row[24],row[25],row[26],row[27]]
              writer_calib.writerow(calib_data)
        if row[0]!='time':
