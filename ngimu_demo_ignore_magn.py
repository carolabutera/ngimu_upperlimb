    ...
    for udp_socket in receive_sockets: 
        try:
            ...
            if calibration_flag==-1: #Settings for calibration 
                ...
                calib_ok=1
                print("Press 'Enter' to start calibration (STEP FORWARD + N-POSE + T-POSE)\n")      
                while(keyboard.read_key() !="enter"): #wait for the patient to press Enter 
                    pass
                print("Make a step forward")
                ...
                if time.time()-start<5:
                   if np.linalg.norm(a_step)<np.linalg.norm(a_TO): #to save the max acceleration
                        a_step=a_TO
                        print(np.linalg.norm(a_step))

                elif (time.time()-start>5) & (time.time()-start<10): #Wait some time to change position
                    if i<1:
                        print("Stand still with arms along sides (N-pose)...\n")
                    i=i+1                      

                elif (time.time()-start>10) & (time.time()-start<20): #N-POSE data acquisiton 
                    i=0
                ...
                elif (time.time()-start>20) & (time.time()-start<25): #Wait some time to change position
                    if i<1:
                        print("Raise arms to the side and keep them horizontal (T-pose)...\n")
                    i=i+1
                    

                elif (time.time()-start>25) & (time.time()-start<35): #T-POSE data acquisition 
                    ...
                    #NB: n-poses are expressed wrt IMU reference frame!

                    UA_tpose=sumUA_tpose/m #mean of the matrix in the t-pose
                    FA_tpose=sumFA_tpose/m
                    #NB: t-poses are expressed wrt IMU reference frame!
                    
                    if relative_angle(TO_npose[:,0].T,a_step)>math.pi/2: 
                        theta=2*math.pi-relative_angle(TO_npose[:,1].T,a_step)
                    else:
                        theta=relative_angle(TO_npose[:,1].T,a_step)
                    
                    #rotation matrix from N-pose to T-pose expressed wrt body reference frame:
                    B_npose_tpose=matrix_op.rotY(-arm*math.pi/2)
                    #rotation matrix from N-pose to T-pose expressed wrt IMU reference frame
                    UA_npose_tpose=np.matmul(UA_tpose,UA_npose.T)
                    FA_npose_tpose=np.matmul(FA_tpose,FA_npose.T)
                    #rotation of the IMU wrt to BODY
                    UA_B=np.matmul(B_npose_tpose,UA_npose_tpose.T)
                    FA_B=np.matmul(B_npose_tpose,FA_npose_tpose.T)
                    TO_B=matrix_op.rotZ(theta)
                    
                    #npose expressed wrt  body reference frame
                    UA_npose_B=np.matmul(UA_B, UA_npose)
                    FA_npose_B=np.matmul(FA_B, FA_npose)
                    TO_npose_B=np.matmul(TO_B, TO_npose)
                ...
                #Calibrated matrix (wrt to NPOSE initial position) expressed wrt to BODY reference fra  
                UA=np.matmul(np.matmul(UA_B,UA_g),UA_npose_B.T)
                FA=np.matmul(np.matmul(FA_B,FA_g),FA_npose_B.T)
                TO=np.matmul(np.matmul(TO_B,TO_g),TO_npose_B.T)
